...
const VoiceToVoiceScreen = () => {
    ...
    const getAudioFromSarvamTTS = async (text, languageCode) => {
        const response = await axios.post(`${BACKEND_URL}/text-to-speech`, {
            text,
            languageCode: languageCode, // Provide the detected language code to backend (which passes to Sarvam)
        });
    ...
    const handleVoiceToVoice = async (audioUri) => {
        ...
        try {
            ...
            const speakInLanguage = async () => {
                ...
                // Use Sarvam TTS for all languages by requesting the backend endpoint
                try {
                    const audioURI = await getAudioFromSarvamTTS(answer, langCode);
                    ...
                    // Fallback to Expo speech for English if Sarvam TTS fails
                    if (langCode.startsWith('en')) {
                        ...
                        alert('Failed to play the response in the detected regional language.');
                    }
                }
            };
